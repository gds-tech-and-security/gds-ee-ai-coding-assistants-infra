...
variable "name" {
  description = "Name to use on IAM role created. If omitted, Terraform will assign a random, unique name"
  type        = string
  default     = null
}
...
variable "attach_policies" {
  description = "Existing policies to attach to the IAM role in `{'static_name' = 'policy_arn'}` format"
  type        = map(string)
  default     = {}
}
...
variable "json_policy_documents" {
  description = <<EOD
    INCOMPATIBLE with 'inline_policy'. Use one or the other
    List of rendered JSON from [aws_iam_policy_document](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) data sources
  EOD
  type        = list(string)
  default     = []
}
...
variable "inline_policy" {
  description = <<EOD
    INCOMPATIBLE with 'json_policy_documents'. Use one or the other
    A map of IAM policy [statements](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document#statement) for inline policy permissions
  EOD
  type = map(object({
    sid           = optional(string)
    actions       = optional(list(string))
